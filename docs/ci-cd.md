...
OCI_PROXY_NETWORK=reverse-proxy-edge
...
The deploy script verifies the shared Caddy container and ingress network before
attaching Swap Assistant's backend. It never deletes or recreates externally
...
existing Caddy ingress network. The shared Caddy container is not given either
database network.
