...
function Ensure-Database {
  ...
  if (-not $dockerAvailable) {
    if (Test-TcpPort -HostName "localhost" -Port 55434) {
      Write-Warning "Docker CLI not found; using existing PostgreSQL on localhost:55434."
      return
    }
    throw "Docker CLI is not installed and PostgreSQL is not reachable on localhost:55434."
  }
  ...
  if (-not (Test-DockerDaemon)) {
    try {
      ...
      if (Test-TcpPort -HostName "localhost" -Port 55434) {
        Write-Warning "$($_.Exception.Message) Using existing PostgreSQL on localhost:55434."
        return
      }
      throw
    }
...
Write-Host "Database: localhost:55434 (wallet / wallet / wallet)"
