...
class LimitOrderServiceTest {
  ...
  void checksFeatureFlagBeforeSaving() {
    ...
    when(submissionClient.submit(eq(ONEINCH_FALLBACK_CHAIN), any(), any(), any(), any()))
        .thenReturn(new LimitOrderSubmissionResult(true, false, "", "provider-order"));
    when(repository.updateSubmissionStatus(eq(stored.id()), eq("submitted"), any(), eq("provider-order")))
        .thenReturn(response("submitted"));
  ...
  void submitsOnlyPayloadThatMatchesRequestTerms() {
    ...
    LimitOrderResponse stored = response("stored");
    when(repository.insert(eq(WALLET), any(), eq("supported"), eq("stored"), any())).thenReturn(stored);
    when(submissionClient.submit(eq(ONEINCH_FALLBACK_CHAIN), eq(request.executionProvider()), eq(request.orderHash()), eq(request.signature()), any()))
        .thenReturn(new LimitOrderSubmissionResult(true, false, "", "provider-order"));
    when(repository.updateSubmissionStatus(eq(stored.id()), eq("submitted"), any(), eq("provider-order")))
        .thenReturn(response("submitted"));
    ...
    verify(submissionClient).submit(eq(ONEINCH_FALLBACK_CHAIN), eq(request.executionProvider()), eq(request.orderHash()), eq(request.signature()), any());
  }
  ...
  private String validPayload(String maker) {
    return """
        ...
        """.formatted(maker, WETH, USDT, WALLET, ONEINCH_FALLBACK_CHAIN);
  }
